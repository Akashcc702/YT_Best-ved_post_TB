...
# Categories from the YouTube API we never want (News=25, Entertainment=24 etc.)
REJECT_CATEGORY_IDS = {"25", "24", "23", "22", "10"}  # News, Entertainment, Comedy, People&Blogs, Music
...
def passes_category_filter(category_id: str | None) -> bool:
    if category_id is None:
        return True
    return category_id not in REJECT_CATEGORY_IDS
...
"""Thin async wrapper around the YouTube Data API v3 (free quota: 10,000 units/day).

Quota cost reference (as of the v3 API):
  search.list  -> 100 units per call
  videos.list  -> 1 unit per call (up to 50 ids per call)

With SEARCH_MAX_CANDIDATES=40 candidates across ~8 queries you'll spend
roughly 8 * 100 (search) + 1 (videos.list, batched) = ~801 units/day —
well inside the free 10,000/day quota with room for /search command usage.
"""
...
ISO8601_DURATION_RE_HOURS = "PT"


def _parse_iso8601_duration(duration: str) -> int:
    """Convert an ISO 8601 duration like 'PT12M34S' to total seconds."""
...
class YouTubeClient:
    ...
    async def search_recent(
        ...
        """Search for videos published within the last `hours_back` hours."""
        published_after = (
        ...
        resp = await self._client.get(f"{YOUTUBE_API_BASE}/search", params=params)
    ...
    async def get_video_details(self, video_ids: list[str]) -> list[dict[str, Any]]:
        """Batch-fetch statistics/contentDetails for up to 50 video ids at a time."""
        ...
        for i in range(0, len(video_ids), 50):
            ...
            resp = await self._client.get(f"{YOUTUBE_API_BASE}/videos", params=params)
            resp.raise_for_status()
